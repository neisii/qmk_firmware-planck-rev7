...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [LR_0] = LAYOUT_planck_mit(
        TD(TD_ESC_CK),  KC_Q,  KC_W,  KC_E,  KC_R,  KC_T,  KC_Y,  KC_U,  KC_I,  KC_O,  KC_P,  KC_BSPC,
        TD(TD_TAB),  KC_A,  KC_S,  KC_D,  KC_F,  KC_G,  KC_H,  KC_J,  KC_K,  KC_L,  KC_SCLN,  KC_QUOT,
        KC_LSFT,  KC_Z,   KC_X,  KC_C,  KC_V,  KC_B,  KC_N,  KC_M,  KC_COMM,  KC_DOT,  KC_SLSH,  KC_ENT,
        AP_GLOB,  TD(TD_MINS_LCTL),  TD(TD_EQL_LOPT),  TD(TD_LGUI_APP),  TT(1),  TD(TD_SPACE_LOCK),  TT(2),  KC_PGDN,  KC_END,  CK_FN,  KC_DEL
    ),

    [LR_1] = LAYOUT_planck_mit(
        KC_TRNS,  KC_1,  KC_2,  KC_3,  KC_4,  KC_5,  KC_6,  KC_7,  KC_8,  KC_9,  KC_0,  KC_TRNS,
        KC_TRNS,  KC_BRMD,  KC_BRMU,  XXXXXXX,  XXXXXXX,  XXXXXXX,  XXXXXXX,  KC_MPRV,  KC_MPLY,  KC_MNXT,  KC_MUTE,  KC_TRNS,
        KC_TRNS,  KC_LEFT,  KC_UP,  KC_DOWN,  KC_RGHT,  KC_TRNS,  KC_VOLD,  KC_VOLU,  KC_F13,  KC_F14,  KC_F15,  KC_TRNS,
...
void mins_lctl_td_finished(tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) { // gui(cmd on mac)
        register_code(KC_LCTL);
    } else if ((get_mods() & MOD_MASK_SHIFT) && state->count == 2) { // 언더스코어
        register_code16(KC_UNDERSCORE);
    } else if (state->count == 2) { // 마이너스
        register_code(KC_MINS);
    }
}
void mins_lctl_td_reset(tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) { // lctl
...
void eql_lopt_td_finished(tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) { // alt(cmd on mac)
        register_code(KC_LALT);
    } else if ((get_mods() & MOD_MASK_SHIFT) && state->count == 2) { // 플러스
        register_code16(KC_PLUS);
    } else if (state->count == 2) { // 이퀄
        register_code(KC_EQL);
    }
}
void eql_lopt_td_reset(tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) { // 메뉴(mouse right click)
...
void lgui_app_td_finished(tap_dance_state_t *state, void *user_data) {
...
void lgui_app_td_reset(tap_dance_state_t *state, void *user_data) {
    // if (ck_fn_on && state->count == 1) {
...
tap_dance_action_t tap_dance_actions[] = {
    ...
    [TD_MINS_LCTL] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, mins_lctl_td_finished, mins_lctl_td_reset),

    [TD_EQL_LOPT] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, eql_lopt_td_finished, eql_lopt_td_reset),

    [TD_LGUI_APP] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, lgui_app_td_finished, lgui_app_td_reset),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
      ...
      case AP_GLOB:// User Defined Functional Keycode
...
// bool encoder_update_user(uint8_t index, bool clockwise) { // 로터리 엔코더(돌리는 노브)의 동작을 제어하는 함수
//   if (muse_mode) {
//     if (IS_LAYER_ON(LR_2)) {
//       if (clockwise) {
//         muse_offset++;
